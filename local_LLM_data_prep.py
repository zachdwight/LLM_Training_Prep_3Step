...
def parse_pdf_to_elements(pdf_path, strategy=UNSTRUCTURED_STRATEGY):
    ...
    try:
        elements = partition_pdf(
            ...
            max_characters=1000, #2000
            new_after_n_chars=800, #1500
            overlap=150,
        )
...
if __name__ == "__main__":
    ...
    for filename in os.listdir(PDF_DIR):
        ...
        suggestions_output_path = os.path.join(OUTPUT_DIR, f"{base_name}_suggestions.json")
        ...
        for i, chunk in enumerate(tqdm(text_chunks_for_qa, desc=f"Processing {filename}")):
